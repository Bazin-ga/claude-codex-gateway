...
> 永久保存每条已捕获的 Claude 对话轮次，并把提示词/回复正文公开给所有能够访问控制台的人；
> 在 `open` 模式下就是 tailnet 中任何能访问控制台的人，没有身份识别，也没有阅读审计。成员
> 标签由本人填写且未经验证。Codex 流量不在对话采集范围内。
...
- **平面分离。** 控制平面私有；只有经过 token 认证的数据平面对外公开。Claude 网关按白名单
  ...
  并对每个设备施加请求量和并发额度。
- **分离的请求遥测与对话。** Claude 网关会持久化请求元数据，以及输入、缓存创建输入、缓存读取
  输入和输出四类 token 数，用于共享指标。指标页面本身不展示正文；P6 会另外永久保存符合条件、
  已捕获的 Claude 对话轮次，供控制台全员浏览。Codex 流量不在这条采集边界内。
