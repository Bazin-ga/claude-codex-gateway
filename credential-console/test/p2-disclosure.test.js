...
test('P6 disclosure is prominent, permanent, and explicit about the open audience', async () => {
  ...
  assert.match(englishNotice, /Hook-enabled Claude Code profiles permanently store the exact\s+client-submitted prompt and final visible assistant response/i);
  assert.match(englishNotice, /Legacy P6 API request\/response\s+fragments remain in a separate diagnostic archive/i);
  assert.match(englishNotice, /open.*anyone on the tailnet.*no identity and no reading audit/is);
  assert.match(englishNotice, /device-asserted and do not authenticate a human/i);
  ...
  assert.match(chineseNotice, /启用 Hook[\s\S]*永久保存[\s\S]*用户提交的提示词[\s\S]*最终可见/);
  assert.match(chineseNotice, /旧 P6[\s\S]*API 请求\/响应片段[\s\S]*诊断归档/);
  assert.match(chineseNotice, /open[\s\S]*tailnet[\s\S]*没有身份识别[\s\S]*阅读审计/i);
  assert.match(chineseNotice, /设备上报[\s\S]*不代表真人身份认证/);
  ...
  for (const english of [files.readmeEn, files.consoleReadme, files.deploy]) {
    assert.match(english, /(?:permanently\s+(?:send|store|stores|retain|retains)[\s\S]{0,180}(?:prompt|round)|API[ -]fragment[\s\S]{0,120}diagnostic)/i);
  ...
  assert.match(files.readmeZh, /永久保存[\s\S]{0,120}用户提交的提示词[\s\S]{0,80}最终可见/);
...
test('real server announces metadata and conversation disclosure before listening', { timeout: 15_000 }, async () => {
  ...
  try {
    ...
    assert.match(privacy.detail, /hook-enabled Claude profiles permanently store exact Claude user-submitted prompts and final visible assistant responses as paired rounds/i);
    assert.match(privacy.detail, /legacy bounded API request\/response fragments remain in a separate diagnostic archive/i);
    assert.match(privacy.detail, /device-asserted and do not authenticate a human/i);
    assert.match(privacy.detail, /visible to every console member/i);
    assert.match(privacy.detail, /open mode anyone (?:on the tailnet )?who can reach the console can read both archives/i);
