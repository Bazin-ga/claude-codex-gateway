...
import { handleMachineControl } from '../lib/machine-control.js';
...
class FakeStore {
  ...
  deviceByToken(token) {
    ...
    return this.active.has(token) ? this.active.get(token) : null;
  }
...
async function harness(t) {
  ...
  const server = http.createServer((req, res) => {
    handleMachineControl(req, res, { store }).catch(() => {
