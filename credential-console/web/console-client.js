...
const translations = {
  ...
  'codex-guard-enable': '低额度保护',
  'codex-guard-threshold': '周额度低于',
  'codex-guard-save': '保存保护设置',
  'codex-guard-help': '低于该阈值后，网关只放行模型名里含',
  'codex-guard-help-tail': '的请求，其余模型一律拒绝。',
  'codex-guard-active': '低额度保护',
  'codex-guard-luna-only': '仅允许 luna 模型',
  ...
  'bedrock-unpinned': '已登记的 Bedrock 账号需要区域、模型 id 和密钥齐备后才能使用。',
  'add-bedrock-heading': '录入 AWS Bedrock 账号',
  'add-bedrock-help': '唯一一个粘贴密钥而不是走授权的 provider：Bedrock API key 在 AWS 控制台签发，没有 OAuth 往返可做。提交后即加密保存，不再展示。登记第一个之后，成员侧才会出现 Bedrock 面板。',
  'bedrock-region': 'AWS 区域',
  'bedrock-model-id': '模型 id（该账号只能调用这一个）',
  ...
  'tab-admin': '管理',
  'admin-heading': '管理',
  'admin-intro': '作用于整个控制台、而不是某个账号或设备的设置。这里的每次改动都会连同操作人一起记入审计日志。',
  'admin-block-heading': '禁用模型',
  'admin-block-intro': '请求被禁用的模型时，网关会在请求到达任何账号之前直接拒绝，调用方看到的是下面填写的提示，而不是模型回答。对 Claude 和 Codex 网关上的所有账号、所有设备生效。',
  'admin-block-patterns': '每个名称都要和完整的模型 ID 一致才算命中，不区分大小写。* 代表任意字符：gpt-5.6* 能匹配 gpt-5.6-sol、gpt-5.6-luna；claude-opus-5 只匹配 claude-opus-5，不会误伤 claude-opus-5-5。多个名称用逗号分隔。',
  'admin-block-scope': '只管得到经过本网关的流量。仍在用本地凭证的机器直接连模型服务商，不受这里的规则影响。',
  'admin-block-none': '目前没有禁用任何模型。',
  'admin-block-add-heading': '新增规则',
  'admin-rule-patterns': '模型名称',
  'admin-rule-message-zh': '提示（中文）',
  'admin-rule-message-en': '提示（英文）',
  'admin-rule-enabled': '立即禁用这些模型',
  'admin-rule-updated': '最后修改',
  'admin-rule-on': '禁用中',
  'admin-rule-off': '已关闭',
  'admin-rule-save': '保存规则',
  'admin-rule-delete': '删除规则',
  'admin-rule-add': '添加规则',
  ...
  'docs-api-codex-models': '当前所选 Codex 账号可用的模型列表。',
  'docs-api-codex': 'Codex 单轮请求，供持有 Codex 网关令牌的设备使用。请求会被计量；安装的 Hook 会把提示词与最终回复配成一轮对话。',
  ...
  'metrics-device-input-comparison-description': '每条线是下界：input、缓存创建 input、缓存读取 input 三类中已知部分之和；整小时一无所知时才留出空档。',
  ...
  'metrics-device-trend-description': '在输入侧已知 Token 与输出 Token 之间切换。',
  ...
  'conversation-captured-turns-heading': '从 API 流量捕获的轮次',
  'conversation-captured-turns-notice': '这些行捕获自网关代理的 API 流量，其中也包含正在进行的对话。Codex 不发送 Claude Code Hook，因此 Codex 对话只会出现在这里。',
  'conversation-captured-turns-filtered': '本页的筛选条件会一并带过去，那边会执行同样的搜索。',
  'conversation-captured-turns-link': '打开已捕获的轮次',
...
document.addEventListener('change', (event) => {
  ...
  if (!select?.matches?.('.machine-filter select[name="account"], .machine-filter select[name="member"], .machine-filter select[name="group"]')) return;
